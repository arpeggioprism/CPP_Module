...
std::string PhoneBook::periodSetter(std::string str)
{
	if (str.length() > 10)
	{
		str = str.substr(0, 9);
...
void PhoneBook::search()
{
	...
	for (int i = 0; i < limit; i++)
	{
		std::cout << std::setw(10) << i
				  << std::setw(13) << periodSetter(contacts[i].getFirstName())
				  << std::setw(13) << periodSetter(contacts[i].getLastName())
				  << std::setw(13) << periodSetter(contacts[i].getNickName()) << std::endl;
	}
	if (contactCount == 0)
	{
